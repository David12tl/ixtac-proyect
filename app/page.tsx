...
// Importamos el InfiniteMenu de forma dinámica ya que usa WebGL/Canvas
const InfiniteMenu = dynamic(() => import('../src/components/ui/InfiniteMenu'), {
  ssr: false,
...
// Datos para la esfera 3D (InfiniteMenu)
const MENU_ITEMS = [
  {
    image: '/fondo-ixtac-4.jpg',
    link: '#mapa',
    title: '500 Escalones',
  ...
  },
  {
    ...
    link: '#mapa',
    title: 'Tuxpanguillo',
    description: 'Valle místico'
  },
  {
    image: 'fondo-2.jpg',
    link: '#mapa',
    title: 'Malacate',
    description: 'Historia industrial'
  },
  {
    image: 'fondo-3.jpg',
    link: '#mapa',
...
const FAQ_DATA = [
  {
    ...
    a: "Ixtaczoquitlán es hermoso todo el año, pero para ver el valle en su máximo esplendor verde y con niebla mística, los meses de junio a septiembre son ideales."
  },
  {
    q: "¿Necesito guía para los senderos?",
    a: "Para rutas básicas como el Mirador, puedes ir por tu cuenta. Para descensos profundos, recomendamos guías locales certificados."
  }
];

// Componente FAQItem (se mantiene igual)
  ...
  return (
    <div className={`border-b border-white/10 transition-all duration-500 ${isOpen ? 'bg-white/5' : ''}`}>
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-64 opacity-100 pb-8' : 'max-h-0 opacity-0'}`}>
        <p className="text-white/60 text-lg font-serif italic leading-relaxed max-w-3xl">{answer}</p>
      </div>
    </div>
  );
};
...
const AttractionCard = ({ title, tag, img, desc }: { title: string, tag: string, img: string, desc: string }) => (
  <div className="group relative h-[450px] w-full rounded-[40px] overflow-hidden border border-white/10 shadow-2xl transition-all duration-500 hover:scale-[1.02]">
    ...
    <div className="absolute inset-0 p-8 flex flex-col justify-end translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
      ...
      <p className="text-white/70 text-sm leading-relaxed mb-6 opacity-0 group-hover:opacity-100 transition-opacity duration-700">
        {desc}
      </p>
  ...
  return (
    <Layout>
      ...
      <div className="fixed inset-0 w-full h-full -z-50 pointer-events-none">
        <Image src="/fondo-ixtac-4.jpg" alt="Background" fill priority quality={100} className="object-cover" />
      ...
      <div className="relative z-10">
        ...
        <main className="max-w-7xl mx-auto px-6 space-y-40 pb-40">
          ...
          {/* SECCIÓN NUEVA: INFINITE MENU (ESFERA 3D) */}
          <section className="relative py-20">
            <div className="text-center mb-10">
              <h2 className="text-white/30 text-[10px] font-black uppercase tracking-[0.5em] mb-4">Exploración Interactiva</h2>
              <h3 className="text-4xl md:text-6xl font-bold text-white tracking-tighter">Gira el Valle</h3>
            </div>
            <div className="h-[500px] md:h-[700px] w-full rounded-[60px] overflow-hidden border border-white/10  backdrop-blur-3xl shadow-inner">
              <InfiniteMenu items={MENU_ITEMS} scale={1.2} />
              <div className="absolute bottom-12 left-1/2 -translate-x-1/2 pointer-events-none">
                <p className="text-[#d4e9c7] text-[10px] font-bold uppercase tracking-[0.2em] animate-pulse">Arrastra para navegar</p>
              </div>
            </div>
          </section>
          ...
          {/* MAPA */}
          <section id="mapa" className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
             <div className="lg:col-span-4 space-y-8 text-white">
                <h2 className="text-5xl font-bold tracking-tighter">Ubica tu ruta</h2>
                <p className="text-lg text-white/70 leading-relaxed font-serif italic">Encuentra cada rincón mágico en nuestro mapa interactivo 3D.</p>
             </div>
